/* =========================================================
   CampusBot — app.js  (Serverless / Direct Gemini API)
   No backend required. API key stored in localStorage.
   ========================================================= */
'use strict';
...
// ── State ─────────────────────────────────────────────────
let apiKey = localStorage.getItem('campusbot_apikey') || '';
let useGemini = false;
let isTyping = false;
let chatHistory = [];
...
changeKeyBtn.addEventListener('click', () => {
  ...
  apiKeyInput.placeholder = 'Paste your API key here (AIza...)';
  showApiModal();
  closeSidebar();
});
...
testBtn.addEventListener('click', async () => {
  if (!apiKey) {
    showToast('⚠️ No API key saved. Add one first.');
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      showToast('✅ Gemini API is working!');
      useGemini = true;
      updateModeUI('gemini');
    } else {
    ...
    showToast('❌ Connection failed. Check your key or internet.');
  }
});
...
clearChatBtn.addEventListener('click', () => {
  if (currentSess.length) { archiveSess(); clearChat(); showToast('Chat cleared ✓'); }
});
newChatBtn.addEventListener('click', () => {
  archiveSess(); clearChat(); closeSidebar(); showToast('New conversation started ✓');
});
...
function appendUser(text) {
  const g = mkEl('div', 'message-group user-group');
...
function appendBot(text, isAI) {
  const g = mkEl('div', 'message-group');
...
function showTyping() {
  ...
  const d = mkEl('div', 'typing-indicator'); d.id = 'typingEl';
...
function renderHistory() {
  ...
  historyList.innerHTML = sessions.map(s =>
    `<div class="history-item"><svg viewBox="0 0 24 24" fill="none"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/></svg>${esc(s.preview)}${s.preview.length >= 45 ? '…' : ''}</div>`
  ).join('');
}
...
function showToast(msg) {
  ...
  if (!t) { t = mkEl('div', 'toast'); document.body.appendChild(t); }
...
function init() {
  ...
  if (apiKey) {
    ...
    // No key saved — show the modal so user can enter one
    showApiModal();
  }
}
